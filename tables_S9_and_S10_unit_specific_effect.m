...
%  - S9,S10: iid errors, signal-to-noise ratio=1, unit-specific effects.
%
% Notes: as in Bonhomme and Manresa (2015) replication package, the results
...
path = append('tables_S9_and_S10/',link);
...
for G=Gseq
    ...
    for idxt=1:4
        ...
        nu = normrnd(0,1,N,nsim);
        for j=1:N
            for simu=1:nsim
                Xmat(j,:,:,simu) = Xmat(j,:,:,simu)+0.2*nu(j,simu);
            end
        end
        ...
        %for simu=1:nsim
            ...
            Y = sum(X.*reshape(kron(beta',ones([N,T])),N,T,K),3)+...
                grp_effects_mat+squeeze(nu(:,simu))+squeeze(err(:,:,simu));
            ...
            residuals = Yd-sum(Xd.*reshape(kron(res_loop_beta_hat3(simu,...
                :)',ones([N,T])),N,T,K),3);
            ...
            [res_loop_beta_hat1(simu,:),alpha_hat1] = FE_reg_withcov(Yd,...
                Xd,res_loop_grp_hat1(simu,:)');
            [var_beta_hat1,~] = compute_GFE_analytical_cov(Yd,Xd,...
            ...
            for j=1:3
                % Clustering
                residuals = Yd-sum(Xd.*reshape(kron(...
            ...
            [betaBIC(Gmax,:),alphaBIC(Gmax,1:Gmax,:),...
                  ...
                  Yd,Xd,Gmax,n_init_GFE);
            G_BIC_list = 1:Gmax-1;
            G_BIC_list = G_BIC_list(G_BIC_list~=G);
            for j=G_BIC_list
                [betaBIC(j,:),alphaBIC(j,1:j,:),...
                  grpBIC(j,:),objBIC(j)] = BM_algo1_multiple_init(Yd,Xd,j,...
                  n_init_GFE);
            end
            ...
            [var_beta_hat8,~] = compute_GFE_analytical_cov(Yd,Xd,...
            ...
            while ~stop
                try
                    [res_loop_beta_hat9(simu,:),~,~,~,~,var_beta_hat9,~,...
                        B1,B2,B3] = LS_factor(Y,XX,G+1,'silent', 1e-8,'m2',...
            ...
            while ~stop
                try
                    [res_loop_beta_hat11(simu,:),~,~,~,~,var_beta_hat11,...
                        ~,B1,B2,B3] = LS_factor(Y,XX,G+1,'silent',1e-8,...
            ...
            [res_loop_beta_hat13(simu,:), alpha_hat13] = ...
                  FE_reg_withcov(Yd,Xd,grp);
            [var_beta_hat13,~] = compute_GFE_analytical_cov(Yd,Xd,...
    ...
    save(append(path,'_res_tableS9_G',string(G),'.mat'),'res_table3');

    % Output raw Table 4 (Full GFE: classification accuracy)
    save(append(path,'_res_tableS10_G',string(G),'.mat'),'res_table4');
   if G==3
...
save(append(path,'_tableS9.mat'),'tab3_out');

tab4_out = round(reshape(mean(res_full_tab4,1),16,15),3);
save(append(path,'_tableS10.mat'),'tab4_out');

% Output LaTex Table S9 (Part 1/2)
fileID = fopen('output_tableS9A_unitFE.tex','w');
...
% Output LaTex Table S9 (Part 2/2)
fileID = fopen('output_tableS9B_unitFE.tex','w');
...
% Output LaTex Tables S10
fileID = fopen('output_tableS10_unitFE.tex','w');
